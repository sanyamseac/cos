	...
	<div class="mb-6 flex items-center justify-between">
		<div>
			<button
				...
				class="mb-2 flex items-center text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300"
			>
		...
		<button
			...
			class="rounded-lg bg-indigo-600 px-4 py-2 font-medium text-white transition-colors hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600"
		>
			Add Money
		</button>
	</div>
	...
	<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
		<div class="lg:col-span-1">
			...
			<div class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
				...
				{:else}
					<p class="py-4 text-center text-gray-500 dark:text-gray-400">No wallet found</p>
				{/if}
			</div>
		</div>
		<!-- Main Content -->
								...
								{#each data.walletTransactions as { transaction, canteen, performedBy }}
									<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
										...
										<td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
											<span
												class={parseFloat(transaction.amount) > 0
