...
<div>
	<div class="mb-3 flex items-center justify-between">
		<h4 class="font-medium text-gray-700 dark:text-gray-300">Addons</h4>
		<Button.Root
			onclick={() =>
			...
			{#each item.addons as addon}
				...
				>
					<div class="flex items-center gap-2">
						...
						<span class="text-sm text-green-600">₹{addon.price}</span>
					...
					<Button.Root
						onclick={() => openCrudModal('addon', true, addon)}
	...
	{:else}
		<p class="text-sm text-gray-500 dark:text-gray-400">No addons configured</p>
	{/if}
