...
<div>
	<div class="mb-3 flex items-center justify-between">
		<h4 class="font-medium text-gray-700 dark:text-gray-300">Variants</h4>
		<Button.Root
			onclick={() =>
			...
			{#each item.variants as variant}
				...
				>
					<div class="flex items-center gap-2">
						...
						<span class="text-sm text-green-600">₹{variant.price}</span>
					...
					<Button.Root
						onclick={() => openCrudModal('variant', true, variant)}
	...
	{:else}
		<p class="text-sm text-gray-500 dark:text-gray-400">No variants configured</p>
	{/if}
	<CrudModal
