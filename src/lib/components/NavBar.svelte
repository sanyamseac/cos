...
<nav
	class="fixed md:sticky bottom-0 md:top-0 z-50 w-full border-b border-gray-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-gray-800 dark:bg-gray-900/95 dark:supports-[backdrop-filter]:bg-gray-900/60"
>
			...
			<NavigationMenu.Root class="relative w-full md:w-auto z-10 flex justify-between inline" orientation="vertical">
				...
				>
					{#each components as component}
						<NavigationMenu.Item>
							<NavigationMenu.Link
								class="hover:text-accent-foreground focus:bg-muted focus:text-accent-foreground data-[state=open]:shadow-mini dark:hover:bg-muted dark:data-[state=open]:bg-muted focus:outline-hidden group inline-flex h-8 w-[18vw] md:w-max py-6 items-center justify-center rounded-[7px] bg-transparent px-4 py-2 text-md font-medium transition-colors hover:bg-white disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-white {page.url.pathname === component.href ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : ''}"
								href={component.href}
					...
					{#if !page.data.user}
						<NavigationMenu.Item>
							<NavigationMenu.Link
								class="hover:text-accent-foreground focus:bg-muted focus:text-accent-foreground data-[state=open]:shadow-mini dark:hover:bg-muted dark:data-[state=open]:bg-muted focus:outline-hidden group inline-flex h-8 w-[18vw] md:w-max py-6 items-center justify-center rounded-[7px] bg-transparent px-4 py-2 text-md font-medium transition-colors hover:bg-white disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-white {page.url.pathname === '/login' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : ''}"
								href="/login"
							>
								<div class="flex flex-col justify-center items-center">
					...
					{:else}
						<NavigationMenu.Item>
							<NavigationMenu.Link
								class="hover:text-accent-foreground focus:bg-muted focus:text-accent-foreground data-[state=open]:shadow-mini dark:hover:bg-muted dark:data-[state=open]:bg-muted focus:outline-hidden group inline-flex h-8 w-[18vw] md:w-max py-6 items-center justify-center rounded-[7px] bg-transparent px-4 py-2 text-md font-medium transition-colors hover:bg-white disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-white {page.url.pathname === '/profile' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : ''}"
								href="/profile"
							>
								<div class="flex flex-col justify-center items-center">
