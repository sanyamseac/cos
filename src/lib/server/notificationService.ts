...
export async function sendOrderNotification(userId: string, orderId: string, status: string, customMessage?: string): Promise<{ success: boolean; sent: number; failed: number }> {
	...
	switch (status) {
		case 'preparing':
			...
			body = customMessage || `Your order #${orderId} is being prepared`
			break
		case 'ready':
			title = '🔔 Order Ready!'
		...
		case 'completed':
			...
			body = customMessage || `Thank you! Order #${orderId} has been completed`
			break
		case 'cancelled':
			title = '❌ Order Cancelled'
			body = customMessage || `Order #${orderId} has been cancelled`
			requireInteraction = true
			break
	}
